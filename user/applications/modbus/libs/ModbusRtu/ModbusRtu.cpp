 ...
 * Initialize class object.
 *
 * Sets up the software serial port using specified baud rate and SoftwareSerial
 * object. Call once class has been instantiated, typically within setup().
 *
 ...
 *
 * Call once class has been instantiated, typically within setup().
 * The time-out timer is reset each time that there is a successful
 * communication between Master and Slave. It works for both.
 *
 ...
 * Avoid any delay() function !!!!
 * After a successful frame between the Master and the Slave, the time-out timer
 * is reset.
 *
...
int8_t Modbus::getRxBuffer()
{
	...
	if (u8serno < 4)
		while (port->available()) {
			...
			if (u8BufferSize >= maximumSizeOfCommunicationBuffer)
				bBuffOverflow = true;
		}
	else
#ifdef MODBUS_ENABLE_SOFTWARE_SERIAL
...
int8_t Modbus::process_FC1(uint16_t *regs)
{
	...
	for (u16currentCoil = 0; u16currentCoil < u16Coilno; u16currentCoil++) {
		...
		bitWrite(au8Buffer[u8BufferSize], u8bitsno, bitRead(regs[u8currentRegister], u8currentBit));
		u8bitsno++;
